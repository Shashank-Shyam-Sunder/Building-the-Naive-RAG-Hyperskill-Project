...
# Stage 8: Completing the RAG Pipeline by appending the user_query and retrieved scenes to the LLM to generate a response.

try:
    user_prompt_generate = ChatPromptTemplate.from_messages([
        ("system",
         "You are an expert on movie scripts. You will answer the question on movies asked by user based on the movie name and the scenes available in the context."
         "You will always generate a detailed and comprehensive answer."),
        ("user",
         "Movie title: {movie_title}\n\nUser query: {query}\n\n Retrieved Scenes for context: {retrieved_scenes}")
    ])

    llm_chain_generation = user_prompt_generate | llm


    def generate_final_answer(llm_chain_in, user_input_in, user_query_in, retrieved_scenes_in):
        """
        Uses an LLM chain to rewrite the user query for better retrieval.
        Returns the rewritten query string.
        """
        llm_response = llm_chain_in.invoke(
            {"movie_title": user_input_in, "query": user_query_in, "retrieved_scenes": retrieved_scenes_in}).content
        return llm_response


    final_response = generate_final_answer(llm_chain_generation, user_input, user_query, retrieved_docs)
    print(f"\n\nFinal response:\n{final_response}")

except Exception as e:
    print(f"Error occurred during response generation from LLM agent: {e}")